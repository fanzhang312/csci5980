var flag_low=false, flag_high=false, start_stop_btn, wpid=false,wpid_high=false, map, z, op,op2, prev_lat, prev_long, min_speed=0, max_speed=0, min_altitude=0, max_altitude=0, distance_travelled=0, min_accuracy=150, date_pos_updated="", info_string="";
...
function geo_success(position)
{
	...
	if(flag_high)
		high_accuracy_btn.innerHTML="Stop"; 
	info_string="";
	...
	if(position.coords.accuracy<=min_accuracy)
	{
			...
			info_string="<br/>Current position: lat=" + prev_lat+", long=" + prev_long+ " (accuracy " +position.coords.accuracy+ "m) <br/>last reading taken at: "+current_datetime;
...
function get_pos()
{
	...
	if(!!navigator.geolocation)
		wpid=navigator.geolocation.watchPosition(geo_success, geo_error, {enableHighAccuracy:false, maximumAge:30000, timeout:27000});
	else
		op.innerHTML="ERROR: Your Browser doesn't support the Geo Location API";
}

function get_pos_highAccuracy()
{
	// Set up a watchPosition to constantly monitor the geo location provided by the browser - NOTE: !! forces a boolean response
	// We  use watchPosition rather than getPosition since it more easily allows (on IPhone at least) the browser/device to refine the geo location rather than simply taking the first available position
	// Full spec for navigator.geolocation can be foud here: http://dev.w3.org/geo/api/spec-source.html#geolocation_interface
	...
	// First, check that the Browser is capable
	if(!!navigator.geolocation)
		wpid_high=navigator.geolocation.watchPosition(geo_success, geo_error, {enableHighAccuracy:true, maximumAge:30000, timeout:27000});
	else
		op2.innerHTML="ERROR: Your Browser doesn't support the Geo Location API";
}
...
function init_geo()
{
	...
	op2=document.getElementById("output_2"); 
	if(op)
	{
		start_stop_btn=document.getElementById("geo_start_stop");
		high_accuracy_btn=document.getElementById("enable_high_accuracy");
		...
		if(start_stop_btn)
		{
			...
			high_accuracy_btn.onclick=function()
			{
				
				if(wpid_high) // If we already have a wpid_high which is the ID returned by navigator.geolocation.watchPosition()
				{
					flag_high = false;
					high_accuracy_btn.innerHTML="High Accuracy Enabled";
					navigator.geolocation.clearWatch(wpid_high);
					wpid_high=false;
				}
				else // Else...We should only ever get here right at the start of the process
				{
					flag_high = true;	
					high_accuracy_btn.innerHTML="Acquiring Geo Location...";
					get_pos_highAccuracy();
				}
			}
